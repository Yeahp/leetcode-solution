...
public class SwordCodeApp {
    ...
    public static int decodeNum(int x) {
    ...
    public static int _decodeNum(int[] arr, int start) {
        if (start < 0) return 0;
        if (start == 0) return 1;
    ...
    public static int cutLineS2(int n) {
        if (n == 2) return 1;
        if (n == 3) return 2;
        int[] arr = new int[n];
        arr[0] = 1;
    ...
    public static ListNode<Integer> deleteListNode(ListNode<Integer> head, ListNode<Integer> node) {
        // 情形1：待删除节点为头节点
        if (head.equals(node)) {
    ...
    public static void zhiShapedBinaryTree(TreeNode<Integer> root) {
        if (root != null) {
    ...
    // 复制链表
    public static ListNode<Integer> copyLinkedList(ListNode<Integer> head) {
        if (head != null) {
    ...
    private static void theFistCharOccurOnce() {
    ...
    public static int noNDigit(int n) {
        if (n <= 9) return n;
        int k = 1;
        while (k > 0) {
            // 确定第 n 个数的位数
    ...
    // m*n 格子，每个格子对应一个正整数，求不同和


    // 丑数: 因子只包含2、3、5的正整数，其中定义 1 为丑数
    // Q1：判断给定的数是否为丑数
    // 循环：
    public static boolean isUglyNumber(int x) {
    ...
    // 递归：
    public static boolean _isUglyNumber(int x) {
        if (x == 1) return true;
        int[] uglyFactor = new int[]{2, 3, 5};
        for (int i : uglyFactor) {
    ...
    // Q2：给定范围内有多少个丑数
    // S1: 逐一验证
    public static int numOfUglyNumberS1(int start, int end) {
        int count = 0;
    ...
    // S2：数学推导
    public static int numOfUglyNumberS2(int start, int end) {
    ...
    private static int intervalJudgeSamePow(int num, int pow) {
        if (num == Math.pow(2, pow)) return 0;
        else if (num < 3 * Math.pow(2, pow - 1)) return 1;
        else return 2;
    }
    private static int intervalJudgeDiffPow(int num, int pow) {
        if (num < 5 * Math.pow(2, pow - 2)) return 0;
        else if (num < 3 * Math.pow(2, pow - 1)) return 1;
        else return 2;
    }
    // Q3：给定 n，返回第 n 个丑数
    public static int noNUglyNumber(int n) {
        if (n == 1) return 1;
        int i = (n - 1) % 3;
    ...
    // S2：二分查找，给数字起/止下标
    public static int numberFrequenceS2(int[] arr, int x) {
        int len = arr.length;
        return _numberFrequence(arr, x, 0, len - 1);
    }
    public static int _numberFrequence(int[] arr, int x, int start, int end) {
    ...
    // 给定一棵二叉树，判断是否为平衡二叉树
    public static boolean isBlanceBinaryTree(TreeNode<Integer> root) {
        if (root == null) return true;
        return Math.abs(maxDepthOfBinaryTree(root.left) - maxDepthOfBinaryTree(root.right)) > 1;
    }
    private static int maxDepthOfBinaryTree(TreeNode<Integer> root) {
    ...
    // S1：借助 subString 函数
    public static String leftRotateStringS1(String s, int i) {
        if (s.length() == i) return s;
        return s.substring(i + 1) + s.substring(0, i + 1);
    }
    // S2：借助数组
    public static String leftRotateStringS2(String s, int i) {
        int len = s.length();
    ...
    public static boolean shunzi(String[] arr) {
        int min = -1;
        int wang = 0;
        int sum = 0;
        for (int i = 0; i < 5; i++) {
    ...
    public static int twoNumSumWithoutAddOp(int i, int j) {
        return 0;
    }
...
//        System.out.println(root.equals(_root));
        ...
        System.out.println(SwordCodeApp.decodeNum(67));
    }
